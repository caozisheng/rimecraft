...
// Layer 1: Identity + constitution rules (slim, always present)
...
// Layer 3: Code structure templates (always present)
...
// Layer 4: Phaser core rules (slim ~80 lines, always present — details in RAG)
...
// Layer 6: Work rules + workflow + error handling (always present)
...
function buildSystemPrompt(params: RunAgentLoopParams): string {
	...
	// Layer 3: Code structure templates
	parts.push(isEn ? CODE_STRUCTURE_EN : CODE_STRUCTURE_ZH);
	parts.push("");

	// Layer 4: Phaser core rules (slim version)
	parts.push(isEn ? PHASER_CORE_RULES_EN : PHASER_CORE_RULES_ZH);
	parts.push("");

	// Layer 5: Dynamic context (game state + RAG)
	...
	// Layer 6: Work rules + error handling
