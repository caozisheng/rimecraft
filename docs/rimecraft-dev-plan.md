...
## 十四、资产持久化存储设计（Tauri + Web 兼容方案）

### 设计目标

- Tauri 桌面端：资产以真实文件存储于 `appDataDir()/projects/{projectId}/assets/`，游戏预览通过 `asset://` 协议零拷贝访问
- Web 端：资产以 Blob 存储于 IndexedDB，通过 `URL.createObjectURL()` 生成临时 URL
- 对外接口保持一致的 Blob 输入/输出，内部根据平台选择最优实现

### StorageProvider 接口变更
...
| 方法 | Web (IndexedDB) | Tauri (fs) |
|------|----------------|------------|
| `readAsset` | IDB → Blob | `readFile(binary)` → `new Blob([u8])` |
| `writeAsset` | Blob → IDB | `Blob.arrayBuffer()` → `writeFile(binary)` |
| `getAssetUrl` | `readAsset` → `URL.createObjectURL()` | `convertFileSrc(localPath)` → `asset://` URL |
...
调用者需在不使用时调用 `URL.revokeObjectURL()` 回收内存。
...
async readAsset(projectId: string, path: string): Promise<Blob> {
  const full = joinPath(root, projectId, "assets", ...path.split("/"));
  const bytes = await readFile(full);  // Uint8Array
...
async getAssetUrl(projectId: string, path: string): Promise<string> {
  const full = joinPath(root, projectId, "assets", ...path.split("/"));
  return convertFileSrc(full);  // → asset://localhost/...
}
```
...
```
# Windows（appDataDir → AppData\Roaming）
C:\Users\{user}\AppData\Roaming\com.rimecraft.desktop\
└── projects/
    └── proj_m4x7k9/                          # projectId
        ├── rimecraft.json                     # 项目元数据 + manifest
        ├── src/
        │   ├── main.ts
        │   ├── config.ts
        │   └── scenes/
        ...
        └── assets/
            ...
            │   └── jump.wav                   # 音效
            ├── tilemaps/
            │   └── level-1.json               # Tiled 地图
            └── fonts/
                └── pixel.ttf                  # 自定义字体

# macOS（appDataDir → ~/Library/Application Support）
~/Library/Application Support/com.rimecraft.desktop/projects/proj_m4x7k9/...

# Linux（appDataDir → ~/.local/share）
~/.local/share/com.rimecraft.desktop/projects/proj_m4x7k9/...
```

> **注意**：Tauri v2 的 `appDataDir()` 在 Windows 上返回 `AppData\Local`（非 `Roaming`），对应 `LOCALAPPDATA` 环境变量。

资产 URL 通过 `convertFileSrc()` 转换：

```
本地路径:  C:\Users\zisheng\AppData\Roaming\com.rimecraft.desktop\projects\proj_m4x7k9\assets\sprites\player.png
asset URL: asset://localhost/C%3A%5CUsers%5Czisheng%5CAppData%5CRoaming%5Ccom.rimecraft.desktop%5Cprojects%5Cproj_m4x7k9%5Cassets%5Csprites%5Cplayer.png
```

**Web 端（IndexedDB）**

```
IndexedDB: rimecraft (version 2)
├── [ObjectStore] projects
│   └── { id: "proj_m4x7k9", meta: {...}, manifest: {...} }
├── [ObjectStore] files
│   ├── { key: "proj_m4x7k9:src/main.ts",          projectId: "proj_m4x7k9", path: "src/main.ts",          content: "..." }
│   ├── { key: "proj_m4x7k9:src/config.ts",         projectId: "proj_m4x7k9", path: "src/config.ts",         content: "..." }
│   └── { key: "proj_m4x7k9:src/scenes/game-scene.ts", projectId: "proj_m4x7k9", path: "src/scenes/game-scene.ts", content: "..." }
├── [ObjectStore] assets
│   ├── { key: "proj_m4x7k9:sprites/player.png",    projectId: "proj_m4x7k9", path: "sprites/player.png",    blob: Blob(12KB) }
│   ├── { key: "proj_m4x7k9:sprites/enemy.png",     projectId: "proj_m4x7k9", path: "sprites/enemy.png",     blob: Blob(8KB) }
│   ├── { key: "proj_m4x7k9:audio/bgm.mp3",         projectId: "proj_m4x7k9", path: "audio/bgm.mp3",         blob: Blob(2.1MB) }
│   └── { key: "proj_m4x7k9:tilemaps/level-1.json", projectId: "proj_m4x7k9", path: "tilemaps/level-1.json", blob: Blob(15KB) }
└── [ObjectStore] user_assets
    └── { id: "ua_...", category: "sprite", source: "builtin", ... }
```

资产 URL 通过 `URL.createObjectURL()` 生成：
...
| 资产类型 | 子目录 | 典型扩展名 |
|---------|--------|-----------|
| 精灵图/图片 | `sprites/` | `.png`, `.jpg`, `.webp` |
| 精灵图集 | `sprites/` | `.json` + `.png` |
| 音频 | `audio/` | `.mp3`, `.wav`, `.ogg` |
| 瓦片地图 | `tilemaps/` | `.json`, `.tmx` |
| 字体 | `fonts/` | `.ttf`, `.woff2` |
| 粒子特效 | `particles/` | `.json` |
| 视频 | `video/` | `.mp4`, `.webm` |
...
- `convertFileSrc` 需要 Tauri capabilities 中配置 `fs:allow-appdata-read-recursive`（已配置）
- `writeFile` (binary) 与 `writeTextFile` (text) 是不同的 API，资产必须用 binary 版本
- Web 端返回的 Object URL 是临时的，页面刷新后失效；Tauri 端 `asset://` URL 是持久的
